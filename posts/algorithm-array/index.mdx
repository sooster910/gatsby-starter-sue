...
여기서의 `k` 는 행과 열이 만나 각자 대응하여 계산하게 될 횟수를 나타낸다. 그래서 k는 사실 row 길이 col길이와도 동등하다고 볼 수 있으므로 $$O(K^3)$$ 의 시간복잡도를 가질 수 있다. (=방법1도 마찬가지이다)
...
```
- 이 방법도 첫번째 방법과 동일하게 `O(n*m*k)` 시간복잡도로 가진다. 
