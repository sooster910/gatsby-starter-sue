...
row1 과 계산될 col1을 가져오는게 아닌, for문 내에서 바로 O(n)계산. 위와 문제 해결 접근은 같지만 계산 수행의 코드를 어디서 하느냐가 조금 다르다. 

1. **행렬 A의 행과 행렬 B의 열을 순회하기 위한 첫 번째 반복문**
    - 행렬 A의 각 행과 행렬 B의 각 열을 순회하기 위해 사용된다.
2. **내부 반복문 (k 반복문)**
    -  두 행렬의 같은 행과 열에 있는 원소들을 순회하여 계산 하는 과정을 거친다. 
3. **행렬 A의 열을 순회하기 위한 두 번째 반복문 또는 행렬 B의 행을 순회하기 위한 두 번째 반복문**
    - 행렬 A의 열 또는 행렬 B의 행을 순회하여 내적(dot product) 계산을 위한 k 반복문을 실행하는 데 사용됩니다. 이것이 행렬 곱셈을 가능하게 하는 핵심이다.
...
2. 이동한 다음 좌표가 주어진 좌표평면의 범위에 유효한지 검사한다.
    1. 만약 유효하지 않다면, 움직일 필요가 없다.
    2. 만약 유효하다면, 방문처리를 한다.
3. 방문처리를 하기 전, 현재 방문한 곳이 이미 방문했던 경로인지 먼저 확인한다.
    1. set의 자료구조를 통해 O(1)의 복잡도로 방문처리에 비용을 많이 들이지 않고 확인할 수 있다. 이미 set에 있다면, 더이상 방문처리할 필요가 없다.
    2.  만약 처음 방문한 곳이라면(set에 없는 경우), (출발 좌표,도착좌표 )(도착좌표,출발좌표)를 추가한다. 
    3. 처음 가본 길이에 +1추가한다.
